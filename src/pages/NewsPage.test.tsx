...
describe("NewsPage", () => {
  ...
  it("initializes state from URL parameters", () => {
    // Test with category and page parameters
    renderWithRouter(<NewsPage />, ["/article?category=イベント&page=2"])

    // Should show event category as active
    const eventButton = screen.getAllByText("イベント")[0]
    expect(eventButton.className).toContain("bg-blue-600")
    expect(eventButton.className).toContain("text-white")
  })

  it("updates URL when category filter is changed", async () => {
    renderWithRouter(<NewsPage />)

    // Click on 'イベント' category
    const eventButton = screen.getAllByText("イベント")[0]
    fireEvent.click(eventButton)
  ...
  it("updates URL when pagination is changed", async () => {
    ...
    // Find and click the next page button (if pagination is available)
    ...
    if (nextButton && !nextButton.closest("button")?.disabled) {
      ...
      await waitFor(() => {
        ...
        expect(screen.queryByText("記事1のタイトル")).not.toBeInTheDocument()
      })
    }
  })

  it("preserves category when changing pages", async () => {
    renderWithRouter(<NewsPage />)
    ...
    // Then try to change page (if pagination exists)
    const nextButton = screen.queryByText("次へ")
    if (nextButton && !nextButton.closest("button")?.disabled) {
      fireEvent.click(nextButton)
